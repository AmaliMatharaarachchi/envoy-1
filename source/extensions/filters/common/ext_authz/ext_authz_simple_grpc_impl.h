...
using ExtAuthzAsyncCallbacks = Grpc::AsyncRequestCallbacks<envoy::service::auth_simple::v3::CheckResponse>;
 ...
 */
class GrpcSimpleClientImpl : public SimpleClient, public ExtAuthzAsyncCallbacks {
public:
  // TODO(gsagula): remove `use_alpha` param when V2Alpha gets deprecated.
  GrpcSimpleClientImpl(Grpc::RawAsyncClientPtr&& async_client,
                 const absl::optional<std::chrono::milliseconds>& timeout, bool use_alpha);
  ~GrpcSimpleClientImpl() override;

  // ExtAuthz::Client
  void cancel() override;
  void check(RequestCallbacks& callbacks, const envoy::service::auth_simple::v3::CheckRequest& request,
             Tracing::Span& parent_span, const StreamInfo::StreamInfo& stream_info) override;

  // Grpc::AsyncRequestCallbacks
  void onCreateInitialMetadata(Http::RequestHeaderMap&) override {}
  void onSuccess(std::unique_ptr<envoy::service::auth_simple::v3::CheckResponse>&& response,
...
private:
  ...
  Grpc::AsyncClient<envoy::service::auth_simple::v3::CheckRequest, envoy::service::auth_simple::v3::CheckResponse>
      async_client_;
