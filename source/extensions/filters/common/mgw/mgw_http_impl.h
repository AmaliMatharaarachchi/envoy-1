...
/**
 * HTTP client configuration for the HTTP authorization (mgw) filter.
 */
class ClientConfig {
public:
  ClientConfig(const envoy::extensions::filters::http::mgw::v3::MGW& config,
...
/**
 * This client implementation is used when the mgw filter needs to communicate with an
